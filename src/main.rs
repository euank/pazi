...
"#);
    ...
    let xdg_dirs = xdg::BaseDirectories::with_prefix("pazi").expect("unable to determine xdg config path");

    let frecency_path = xdg_dirs.place_config_file("pazi_dirs.msgpack").expect("could not create xdg 'pazi_dirs.msgpack' path");
    let frecency_file = std::fs::OpenOptions::new().read(true).write(true).create(true).open(frecency_path.clone()).unwrap();
    ...
    if let Some(dir) = flags.value_of("add-dir") {
        ...
        let frecency_update_path = xdg_dirs.place_config_file(format!(".pazi_dirs.msgpack.{}", my_pid)).expect("could not create xdg 'pazi_dirs.msgpack' path");
        let frecency_update_file = std::fs::File::create(frecency_update_path.clone()).unwrap();

        frecency.serialize(&mut rmp_serde::Serializer::new(frecency_update_file)).unwrap();
